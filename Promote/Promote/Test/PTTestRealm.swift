...
/**
 ...
 2. Realm 模型属性必须使用 @objc dynamic var 特性，从而让其能够访问底层数据库的数据。注意，如果这个类 被 @objcMembers 所声明（Swift 4 以及之后的版本），那么各个属性可以只使用 dynamic var 来声明。
...
@objcMembers class PTTestRealmPeopleModel: Object {
    ...
    dynamic var name = ""
    dynamic var age = 0
    dynamic var learnOritation = ""
    dynamic var isLikeUS: RealmOptional<Bool>?
    dynamic var haveGoCountry: String?
    
    
//    var part: PTTestRealmPartModel?
    // 建立外键关联\互联。好像外部不能直接初始化使用，这里使用setValueforkey处理
     dynamic var part = LinkingObjects.init(fromType: PTTestRealmPartModel.self, property: "peoples")
...
@objcMembers class PTTestRealmPartModel: Object {
    ...
    dynamic var id = UUID().uuidString
    dynamic var name = ""
    dynamic var peoples = List<PTTestRealmPeopleModel>()
